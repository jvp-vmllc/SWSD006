...
LOG_MODULE_REGISTER(lora, CONFIG_SIDEWALK_LOG_LEVEL);
...
static void radio_to_lr11xx_lora_modulation_params(lr11xx_radio_mod_params_lora_t *sx_m, const sid_pal_radio_lora_modulation_params_t *rd_m)
{
...
static void radio_to_lr11xx_lora_packet_params(lr11xx_radio_pkt_params_lora_t *sx_p, const sid_pal_radio_lora_packet_params_t *rd_p)
{
    sx_p->preamble_len_in_symb = rd_p->preamble_length;

    _Static_assert(LR11XX_RADIO_LORA_PKT_EXPLICIT == SID_PAL_RADIO_LORA_HEADER_TYPE_VARIABLE_LENGTH, "Invalid convertion");
    _Static_assert(LR11XX_RADIO_LORA_PKT_IMPLICIT == SID_PAL_RADIO_LORA_HEADER_TYPE_FIXED_LENGTH, "Invalid convertion");
...
int32_t sid_pal_radio_set_lora_sync_word(uint16_t sync_word)
{
    ...
    if (drv->ver.fw >= NOT_FULL_FUNCTIONING_VERSION) {
        if (lr11xx_radio_set_lora_sync_word(drv, syncword) != LR11XX_STATUS_OK) {
            return RADIO_ERROR_HARDWARE_ERROR;
        }
    } else {
        if (lr11xx_radio_set_lora_public_network(drv, syncword) != LR11XX_STATUS_OK) {
            return RADIO_ERROR_HARDWARE_ERROR;
        }
    }
...
int32_t sid_pal_radio_set_lora_symbol_timeout(uint8_t num_of_symbols)
{
	...
	sid_pal_radio_lora_modulation_params_t *mp;
	mp = &drv->settings_cache.lora_mod_params;
	drv->suppress_rx_timeout.lora_timeout_us = radio_lora_symb_time[mp->bandwidth - 4][12 - mp->spreading_factor] * (num_of_symbols+1);
	drv->suppress_rx_timeout.lora_timeout_us += radio_lora_rx_base_delay_us[12 - mp->spreading_factor];
	drv->suppress_rx_timeout.lora_timeout_us += radio_lora_rx_input_delay_us[mp->bandwidth - 4];
	num_of_symbols = 0;
#endif /* LORA_SUPPRESS_RX_TIMEOUT */
    lr11xx_status_t status = lr11xx_radio_set_lora_sync_timeout(drv, num_of_symbols);
...
int32_t sid_pal_radio_set_lora_cad_params(const sid_pal_radio_lora_cad_params_t *cad_params)
{
    ...
    if (lr11xx_radio_set_cad_params(drv, &lora_cad_params) != LR11XX_STATUS_OK) {
        return RADIO_ERROR_HARDWARE_ERROR;
    }
