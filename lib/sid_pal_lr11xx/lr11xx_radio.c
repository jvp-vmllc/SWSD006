...
/* TODO include actual LBM header ? */
void smtc_modem_hal_radio_irq(void);
#endif /* SMTC_MODEM_HAL_IRQ_FROM_SID_PAL */

#define LR11XX_DEFAULT_LORA_IRQ_MASK	   (LR11XX_SYSTEM_IRQ_ALL_MASK & ~(LR11XX_SYSTEM_IRQ_PREAMBLE_DETECTED | \
...
LOG_MODULE_REGISTER(lr11xx_radio, CONFIG_SIDEWALK_LOG_LEVEL);
...
void lr11xx_restore_transceiver()
{
	...
	if (lr11xx_radio_set_pkt_type(&drv_ctx, pkt_type) != LR11XX_STATUS_OK) {
		LOG_ERR("lr11xx_radio_set_pkt_type failed");
	}

	if ((err = radio_enable_irq(&drv_ctx)) != RADIO_ERROR_NONE) {
		LOG_ERR("%d = radio_enable_irq", err);
	}
}
...
static inline uint32_t us_to_rtc_ticks(uint32_t us)
{
	...
	case 0:
		return 0;
	default:
		return (((us)*(LR11XX_RTC_FREQ_IN_HZ/COMMON_DIVIDER))/(US_IN_SEC/COMMON_DIVIDER));
	}
}

#define GNSS_TXCO_DELAY_US		15000
/* RALBSP controlling scan state */
void sid_pal_scan_mode(bool en)
{
	if (en) {
...
static int32_t radio_lr11xx_platform_init(void)
{
	...
	} else LOG_ERR("client_selector not connected");


	if (drv_ctx.config->gpios.int1 != HALO_GPIO_NOT_CONNECTED) {
		LOG_INF("int1 %d", drv_ctx.config->gpios.int1);
		if (sid_pal_gpio_set_direction(drv_ctx.config->gpios.int1,
			SID_PAL_GPIO_DIRECTION_INPUT) != SID_ERROR_NONE) {
			goto ret;
		}
	} else LOG_ERR("gpios int1 NC");
	...
	if (drv_ctx.config->gpios.led_rx != HALO_GPIO_NOT_CONNECTED) {
		if (sid_pal_gpio_set_direction(drv_ctx.config->gpios.led_rx,
			SID_PAL_GPIO_DIRECTION_OUTPUT) != SID_ERROR_NONE) {
			goto ret;
		}
	} else LOG_WRN("led_rx NC");

	if (drv_ctx.config->gpios.led_tx != HALO_GPIO_NOT_CONNECTED) {
		if (sid_pal_gpio_set_direction(drv_ctx.config->gpios.led_tx,
			SID_PAL_GPIO_DIRECTION_OUTPUT) != SID_ERROR_NONE) {
			goto ret;
		}
	} else LOG_WRN("led_tx NC");

	if (drv_ctx.config->gpios.led_sniff != HALO_GPIO_NOT_CONNECTED) {
		if (sid_pal_gpio_set_direction(drv_ctx.config->gpios.led_sniff,
			SID_PAL_GPIO_DIRECTION_OUTPUT) != SID_ERROR_NONE) {
			goto ret;
		}
	} else LOG_WRN("led_sniff NC");

	if (drv_ctx.config->gpios.gnss_lna != HALO_GPIO_NOT_CONNECTED) {
		if (sid_pal_gpio_set_direction(drv_ctx.config->gpios.gnss_lna,
			SID_PAL_GPIO_DIRECTION_OUTPUT) != SID_ERROR_NONE) {
			goto ret;
		}
	} else LOG_WRN("gnss_lna NC");

#ifndef TIMING_TEST
	if (drv_ctx.config->gpios.debug_A != HALO_GPIO_NOT_CONNECTED) {
		if (sid_pal_gpio_set_direction(drv_ctx.config->gpios.debug_A,
			SID_PAL_GPIO_DIRECTION_OUTPUT) != SID_ERROR_NONE) {
			goto ret;
		}
		sid_pal_gpio_write(drv_ctx.config->gpios.debug_A, 0);
	} else LOG_WRN("debug_A NC");
#endif /* !TIMING_TEST */
...
static void radio_irq(uint32_t pin, void * callback_arg)
{
	...
	sid_pal_gpio_write(drv_ctx.config->gpios.debug_A, 1);  // indicate radio_irq() start

	uint8_t pinState;
	if (sid_pal_gpio_read(pin, &pinState) == SID_ERROR_NONE) {
		if (pinState) {
			drv_ctx.inhibit_sleep = true;
#ifdef FSK_SUPPRESS_RX_TIMEOUT
	#ifdef LORA_SUPPRESS_RX_TIMEOUT
	...
	#endif /* !LORA_SUPPRESS_RX_TIMEOUT */
#else
			sid_clock_now(SID_CLOCK_SOURCE_UPTIME, &drv_ctx.radio_rx_packet->rcv_tm, NULL);
	...
	sid_pal_gpio_write(drv_ctx.config->gpios.debug_A, 0);  // indicate radio_irq() end
}
...
static int32_t radio_set_irq_mask(lr11xx_system_irq_mask_t irq_mask)
{
   	if (drv_ctx.radio_state == SID_PAL_RADIO_SCAN) {
		LOG_WRN("drop set_irq_mask");
		return RADIO_ERROR_NONE;
	}
	if (lr11xx_system_set_dio_irq_params(&drv_ctx, irq_mask, LR11XX_SYSTEM_IRQ_NONE)
			!= LR11XX_STATUS_OK) {
...
{
   	if (drv_ctx.radio_state == SID_PAL_RADIO_SCAN) {
		LOG_WRN("drop clear all irq");
		return RADIO_ERROR_NONE;
	}
	uint8_t pinState;
	if (sid_pal_gpio_read(drv_ctx.config->gpios.int1, &pinState) == SID_ERROR_NONE) {
		if (!pinState)
			return RADIO_ERROR_NONE;	 // no need to clear anything
	} else
		return RADIO_ERROR_HARDWARE_ERROR;

	if (lr11xx_system_clear_irq_status(&drv_ctx, LR11XX_SYSTEM_IRQ_ALL_MASK)
			!= LR11XX_STATUS_OK) {
		return RADIO_ERROR_HARDWARE_ERROR;
	}
	return RADIO_ERROR_NONE;
...
{
	const lr11xx_status_t status =  lr11xx_system_set_dio_irq_params(drv,
																	drv_ctx.irq_mask,
...
int32_t radio_lr11xx_set_radio_mode(bool rf_en, bool tx_en, bool ext_pa)
{
	int32_t err = RADIO_ERROR_NONE;
   	...
   	if (drv_ctx.radio_state == SID_PAL_RADIO_SCAN) {
		LOG_WRN("during-scan radio_mode");
	}
...
int32_t sid_pal_radio_irq_process(void)
{
...
#endif
			// carrier sense done, restore enabled interrupts for normal operation
		}
...
#ifdef SMTC_MODEM_HAL_IRQ_FROM_SID_PAL
	if (lr11xx_system_get_irq_status(&drv_ctx, &irq_status) != LR11XX_STATUS_OK) {
		return RADIO_ERROR_HARDWARE_ERROR;
	}
	if (irq_status & (LR11XX_SYSTEM_IRQ_ERROR | LR11XX_SYSTEM_IRQ_GNSS_SCAN_DONE | LR11XX_SYSTEM_IRQ_WIFI_SCAN_DONE)) {
		/* smtc_modem_hal to service this instead of here */
		if (irq_status & (LR11XX_SYSTEM_IRQ_GNSS_SCAN_DONE | LR11XX_SYSTEM_IRQ_WIFI_SCAN_DONE)) {
			drv_ctx.post_scan_recovery = true;
				// ? TODO necessary to set a flag, can radio be recovered here ? */
			/*drv_ctx.restore_radio = true;
			drv_ctx.restore_irq_mask = true;*/
		}
	...
	do {
		...
		if (irq_status & LR11XX_SYSTEM_IRQ_ERROR) {
			...
			LOG_WRN("LR1110: Errors 0x%.4X", errors);
		}

		if (irq_status & LR11XX_SYSTEM_IRQ_TX_DONE) {
			TIMING_TEST_SEND_STR("\x08", 1);
...
#ifdef FSK_SUPPRESS_RX_TIMEOUT
			if (drv_ctx.suppress_rx_timeout.timer_armed) {
				/* rx timeout was not suppressed */
				LOG_ERR("rxTimeout during timer");
			}
#endif /* FSK_SUPPRESS_RX_TIMEOUT */
		...
		if (drv_ctx.modem == SID_PAL_RADIO_MODEM_MODE_LORA) {
			...
			if ((irq_status & LR11XX_SYSTEM_IRQ_RX_DONE) && !(irq_status & LR11XX_SYSTEM_IRQ_CRC_ERROR) && DUAL_LINK_SUPPORT) {
				TIMING_TEST_SEND_STR("\x06", 1);
				sid_pal_gpio_write(drv_ctx.config->gpios.led_rx, 0);
		...
		if (irq_status & LR11XX_SYSTEM_IRQ_WIFI_SCAN_DONE ) {
			/* should LBM middleware handle this? */
			LOG_WRN("IRQ_WIFI_SCAN_DONE ");
		}
		if (irq_status & LR11XX_SYSTEM_IRQ_GNSS_SCAN_DONE) {
			/* should LBM middleware handle this? */
			LOG_WRN("IRQ_GNSS_SCAN_DONE");
		}

		if (irq_status & LR11XX_SYSTEM_IRQ_CMD_ERROR) {
			LOG_ERR("IRQ_CMD_ERROR %x", drv_ctx.last.failedCommand);
			if (drv_ctx.radio_state == SID_PAL_RADIO_SCAN) {
				/* ? TODO stop scan ? */
				LOG_ERR("___cmd_err during scan___");
			}
		}
	} while(0);
...
int32_t sid_pal_radio_set_frequency(uint32_t freq)
{
	...
	do {
		...
		if (cur_freq_band != freq_band) {
			if(lr11xx_system_calibrate_image(&drv_ctx, freq_band->start / FREQ_STEP, freq_band->stop / FREQ_STEP) != LR11XX_STATUS_OK) {
				err = RADIO_ERROR_HARDWARE_ERROR;
				break;
			}
		}
		...
		#if RADIO_LR11XX_TXPWR_WORKAROUND
		if (freq_band->start == LR11XX_BAND_EDGE_BAND_START_FREQ &&
			drv_ctx.modem == SID_PAL_RADIO_MODEM_MODE_LORA) {
			uint32_t reg_val;
			...
			if (lr11xx_regmem_write_regmem32_mask(&drv_ctx, LR11XX_REG_ADDR_BAND_EDGE_FIX,
				...
				LOG_WRN("LR1110: Band edge fix was not applied.");
				err = RADIO_ERROR_HARDWARE_ERROR;
				break;
			}
		}
...
int32_t sid_pal_radio_set_tx_power(int8_t power)
{
	...
	if (err == RADIO_ERROR_NONE) {
		...
		do {
			...
			if (lr11xx_radio_set_tx_params(&drv_ctx, drv_ctx.pa_cfg.tx_power_in_dbm,
				  drv_ctx.pa_cfg.ramp_time) != LR11XX_STATUS_OK) {
				break;
			}
			err = RADIO_ERROR_NONE;
		} while (0);
...
{
	int32_t err;
	TIMING_TEST_SEND_STR("\x05", 1);
   	if (drv_ctx.radio_state == SID_PAL_RADIO_SCAN || drv_ctx.inhibit_sleep) {
		return RADIO_ERROR_NONE;
	}
...
{
	int32_t err;
	TIMING_TEST_SEND_STR("\x03", 1);
	if (drv_ctx.radio_state == SID_PAL_RADIO_SCAN) {
		LOG_WRN("drop start_tx");
		return RADIO_ERROR_BUSY;
	}
...
int32_t sid_pal_radio_start_rx(uint32_t timeout)
{
	...
	char str[] = {0x02, rxCnt >> 8, rxCnt & 0xff};
	TIMING_TEST_SEND_STR(str, 3);
	rxCnt++;
#endif /* TIMING_TEST */
	if (drv_ctx.radio_state == SID_PAL_RADIO_SCAN) {
		...
		if (sid_pal_timer_arm(&lr_timer, SID_PAL_TIMER_PRIO_CLASS_PRECISE, &first_shot, NULL) == SID_ERROR_NONE) {
			drv_ctx.rx_timeout_sim = true;
			return RADIO_ERROR_NONE;
		} else {
			LOG_ERR("fake rx timeout %u fail", timeout);
	...
	do {
		...
		//timeout = drv_ctx.suppress_rx_timeout.lora_timeout_us;
	#else
		if (drv_ctx.modem == SID_PAL_RADIO_MODEM_MODE_FSK)
			rtc_ticks = LR11XX_RX_CONTINUOUS_VAL;
		else
...
#ifdef FSK_SUPPRESS_RX_TIMEOUT
#ifndef LORA_SUPPRESS_RX_TIMEOUT
		if (drv_ctx.modem == SID_PAL_RADIO_MODEM_MODE_FSK)
#endif /* !LORA_SUPPRESS_RX_TIMEOUT */
		{
			struct sid_timespec first_shot, tm2;
			timeout += 450;	// duration from radio interrupt assertion to end of sid_pal_radio_irq_process()
			sid_clock_now(SID_CLOCK_SOURCE_UPTIME, &first_shot, NULL);
			tm2.tv_sec = 0;
			tm2.tv_nsec = timeout * 1000;
			sid_time_add(&first_shot, &tm2); /* Add tm1 and tm2, set result in tm1 */
			if ((err = sid_pal_timer_arm(&lr_timer, SID_PAL_TIMER_PRIO_CLASS_PRECISE, &first_shot, NULL)) != SID_ERROR_NONE) {
				LOG_ERR("_arm_timer sid_pal_timer_arm fail %u", timeout);
				break;
			} else
				drv_ctx.suppress_rx_timeout.timer_armed = true;
		}
#endif /* FSK_SUPPRESS_RX_TIMEOUT */
...
int32_t sid_pal_radio_start_carrier_sense(uint32_t timeout, sid_pal_radio_cad_param_exit_mode_t exit_mode)
{
	...
	do {
		...
		if ((err = radio_set_irq_mask(LR11XX_SYSTEM_IRQ_ALL_MASK))
				!= RADIO_ERROR_NONE) {
			break;
		}
...
int32_t sid_pal_radio_start_continuous_rx(void)
{
	...
	do {
		...
		sid_pal_gpio_write(drv_ctx.config->gpios.led_rx, 1);
		if (lr11xx_radio_set_rx_with_timeout_in_rtc_step(&drv_ctx, LR11XX_RX_CONTINUOUS_VAL)
				!= LR11XX_STATUS_OK) {
			err = RADIO_ERROR_HARDWARE_ERROR;
			break;
		}
...
int32_t sid_pal_radio_set_rx_duty_cycle(uint32_t rx_time, uint32_t sleep_time)
{
	...
	do {
		...
		sid_pal_gpio_write(drv_ctx.config->gpios.led_rx, 1);
		if (lr11xx_radio_set_rx_duty_cycle(&drv_ctx, rx_time, sleep_time, LR11XX_RADIO_RX_DUTY_CYCLE_MODE_CAD) != LR11XX_STATUS_OK) {
			err = RADIO_ERROR_HARDWARE_ERROR;
			break;
		}
...
{
#if defined(FSK_SUPPRESS_RX_TIMEOUT)
	uint8_t pinState;
	drv_ctx.suppress_rx_timeout.timer_armed = false;
	if (sid_pal_gpio_read(drv_ctx.config->gpios.int1, &pinState) == SID_ERROR_NONE) {
		if (!pinState) {
...
int32_t sid_pal_radio_init(sid_pal_radio_event_notify_t notify, sid_pal_radio_irq_handler_t dio_irq_handler, sid_pal_radio_rx_packet_t *rx_packet)
{
	...
	if (drv_ctx.radio_state == SID_PAL_RADIO_SCAN) {
		LOG_ERR("radio_init during gnss scan");
	...
	do {
		...
		LOG_INF("LR1110: VER HW 0x%.2X FW 0x%.4X type %x", drv_ctx.ver.hw, drv_ctx.ver.fw, drv_ctx.ver.type);
		...
		if ((err = sid_pal_gpio_input_mode(drv_ctx.config->gpios.int1, SID_PAL_GPIO_INPUT_CONNECT)) != SID_ERROR_NONE) {
			break;
		}
		uint8_t pinState;
		if ((err = sid_pal_gpio_read(drv_ctx.config->gpios.int1, &pinState)) != SID_ERROR_NONE) {
			break;
		}
		...
		if (sid_pal_gpio_set_irq(drv_ctx.config->gpios.int1,
			...
			err = RADIO_ERROR_IO_ERROR;
			LOG_ERR("sid_pal_gpio_set_irq fail");
			break;
