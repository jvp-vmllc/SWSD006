...
LOG_MODULE_REGISTER(sx126x_radio, CONFIG_SIDEWALK_LOG_LEVEL);
        ...
        if (drv_ctx.modem == SID_PAL_RADIO_MODEM_MODE_LORA) {
            ...
            if (irq_status & SX126X_IRQ_RX_DONE) {
                ...
                if ((err = radio_fsk_process_rx_done(&drv_ctx, &fsk_rx_done_status)) == RADIO_ERROR_NONE) {
					...
					LOG_WRN("fsk_rx_done_status %d", fsk_rx_done_status);
					...
					LOG_ERR("%d = radio_fsk_process_rx_done()", err);
				}
            }
        }
    } while(0);
...
int32_t sid_pal_radio_sleep(uint32_t sleep_us)
{
    ...
    do {
        ...
        if ((err = radio_clear_irq_status_all()) != RADIO_ERROR_NONE) { // jun3
            break;
        }
...
int32_t sid_pal_radio_start_carrier_sense(uint32_t timeout,
                                          sid_pal_radio_cad_param_exit_mode_t exit_mode)
{
    int32_t err;
    ...
    do {
        ...
        //nowhere is this read --> drv_ctx.settings_cache.fsk_cad_params = *cad_params;
        drv_ctx.radio_state = SID_PAL_RADIO_RX;
